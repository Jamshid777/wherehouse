...
import { ClientPaymentFormModal } from './forms/ClientPaymentFormModal';
...
export const ClientPaymentsView: React.FC<ClientPaymentsViewProps> = ({ dataManager }) => {
  const { clientPayments, clients, addClientPayment } = dataManager;
  ...
  const filteredPayments = useMemo(() => {
    ...
    return clientPayments.filter(note => {
  ...
  const handleSubmit = (formData: Omit<Payment, 'id' | 'doc_number' | 'links'>) => {
    addClientPayment(formData as any); // Adapt type
    handleCloseModal();
  };
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-md">
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <h2 className="text-2xl font-bold text-slate-800">Mijoz To'lov Hujjatlari</h2>
        <button
        ...
        <table className="w-full text-sm text-left border-collapse">
          <thead className="text-xs text-slate-500 uppercase bg-slate-50 tracking-wider">
            <tr>
              ...
              <th scope="col" className="px-6 py-3 border-r border-slate-200">Mijoz</th>
          ...
          <tbody className="divide-y divide-slate-200">
            {filteredPayments.map(note => (
              <tr key={note.id} className="hover:bg-slate-50">
                ...
                <td className="px-6 py-4 text-slate-600 border-r border-slate-200">{clients.find(s => s.id === note.client_id)?.name || 'Noma\'lum'}</td>
