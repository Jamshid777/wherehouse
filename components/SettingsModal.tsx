...
type ActiveTab = 'general' | 'warehouses';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, dataManager, defaultWarehouseId, setDefaultWarehouseId, appMode, setAppMode }) => {
    ...
    const tabs: {id: ActiveTab, label: string, icon: React.FC<any>}[] = [
        {id: 'general', label: 'Umumiy sozlamalar', icon: SettingsIcon},
        {id: 'warehouses', label: 'Omborlarni boshqarish', icon: WarehouseIcon},
    ]

    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Sozlamalar" size="4xl">
            ...
            {activeTab === 'warehouses' && (
                <WarehousesView dataManager={dataManager} />
            )}
        </Modal>
    );
};
