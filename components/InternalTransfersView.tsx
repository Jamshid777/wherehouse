        ...
        <table className="w-full text-sm text-left">
          <thead className="text-xs text-slate-500 uppercase bg-slate-50 tracking-wider">
            <tr>
              ...
              <th scope="col" className="px-6 py-3">Jo'natuvchi</th>
              <th scope="col" className="px-6 py-3">Qabul Qiluvchi</th>
          ...
          <tbody className="divide-y divide-slate-200">
            {filteredInternalTransfers.map(note => (
              <tr key={note.id} className="hover:bg-slate-50">
                ...
                <td className="px-6 py-4 text-slate-600">{warehouses.find(w => w.id === note.from_warehouse_id)?.name || 'Noma\'lum'}</td>
                <td className="px-6 py-4 text-slate-600">{warehouses.find(w => w.id === note.to_warehouse_id)?.name || 'Noma\'lum'}</td>
            ...
            <form onSubmit={handleFormSubmit} className="space-y-6">
                ...
                <div className="border-t pt-4">
                    ...
                    <div className="overflow-x-auto">
                     ...
                     <button type="button" onClick={handleAddItem} disabled={!formData.from_warehouse_id || !!error} className="mt-4 flex items-center gap-2 text-sm text-blue-600 hover:text-blue-800 font-medium disabled:text-slate-400 disabled:cursor-not-allowed">
                ...
                <div className="flex justify-end gap-3 pt-4 border-t">
                    ...
                    <button type="submit" disabled={!!error} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 disabled:bg-blue-300">Saqlash</button>
                </div>
            </form>
        </Modal>
    );
